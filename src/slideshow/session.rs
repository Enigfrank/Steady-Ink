...
mod tests {
    ...
    /// 创建测试用可靠放映页快照。
    fn page(position: u32) -> SlidePage {
        SlidePage::new(
            PageKey::new(position).expect("测试页码必须大于零"),
            Some(i64::from(position)),
            Some(2),
        )
    }

    /// 验证翻页后返回原位置会恢复该位置自己的墨迹。
    #[test]
    fn switching_pages_restores_per_position_ink() {
        let key = SlideShowKey::new(PresentationApplication::PowerPoint, "deck-a", 7);
        let mut session = SlideShowSession::new(key, page(1));
        session.current_document_mut().append_draw_stroke(
            vec![CanvasPoint::new(10.0, 10.0)],
            InkColor::Red,
            PenWidth::Px8,
        );

        session.switch_page(page(2));
        assert!(session.current_document().has_no_history());
        session.current_document_mut().append_draw_stroke(
            vec![CanvasPoint::new(20.0, 20.0)],
            InkColor::Blue,
            PenWidth::Px8,
        );

        session.switch_page(page(1));
        assert_eq!(session.current_document().operations().len(), 1);
        assert_eq!(session.saved_page_count(), 1);
    }

    /// 验证不可靠总页数会完全隐藏页码区域。
    #[test]
    fn page_numbers_require_both_values_to_be_reliable() {
        let unknown_total = SlidePage::new(PageKey::new(1).expect("有效页键"), None, None);
        assert_eq!(unknown_total.reliable_page_numbers(), None);
        assert_eq!(page(1).reliable_page_numbers(), Some((1, 2)));
    }
}
