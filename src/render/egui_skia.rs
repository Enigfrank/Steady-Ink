...
mod tests {
    ...
    #[test]
    fn transparent_vertex_color_stays_transparent() {
        assert_eq!(
            unpremultiplied_skia_color(egui::Color32::TRANSPARENT),
            Color::TRANSPARENT
        );
    }
    ...
    /// 验证半透明预乘颜色能够恢复原始颜色分量。
    #[test]
    fn premultiplied_vertex_color_is_restored() {
        let color = egui::Color32::from_rgba_unmultiplied(200, 100, 50, 128);
        let result = unpremultiplied_skia_color(color);
    ...
    #[test]
    fn mixed_font_mesh_splits_white_and_textured_triangles() {
        ...
        assert_eq!(groups[1].indices, vec![0, 1, 2]);
    }
}
