...
mod tests {
    ...
    /// 验证完整设置页使用 100% 尺寸，同时工具界面保持既有 80%。
    #[test]
    fn settings_metrics_are_full_size_without_changing_tool_metrics() {
        assert_eq!(TOOL_METRICS.scale, 0.8);
        assert_eq!(SETTINGS_METRICS.scale, 1.0);
        assert!((TOOL_METRICS.touch_target - 51.2).abs() < f32::EPSILON);
        assert_eq!(SETTINGS_METRICS.touch_target, 64.0);
        assert_eq!(SETTINGS_METRICS.margin_space_4, 16);
        assert_eq!(SETTINGS_METRICS.card_radius, 8);
    }

    /// 验证设置样式每帧都从相同 80% 基线恢复，不会产生累计放大。
    #[test]
    fn settings_builtin_style_scaling_is_repeatable_from_tool_baseline() {
        let mut tool_style = Style::default();
        scale_builtin_style(&mut tool_style, INTERFACE_SCALE as f32);
        let mut first = tool_style.clone();
        let mut second = tool_style.clone();
        let ratio = SETTINGS_INTERFACE_SCALE / INTERFACE_SCALE as f32;

        scale_builtin_style(&mut first, ratio);
        scale_builtin_style(&mut second, ratio);

        assert_eq!(first.spacing.interact_size, second.spacing.interact_size);
        assert_eq!(
            first.spacing.scroll.bar_width,
            second.spacing.scroll.bar_width
        );
